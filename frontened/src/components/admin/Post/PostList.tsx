...
const PostList = () => {
    ...
    useEffect(()=>{
        adminaxiosInstance.get('/allPost').then((res)=>{
    ...
    const handleDelete = async (id: string) => {
        await adminaxiosInstance.delete(`/delete/${id}`).then((res) => {
          console.log(res);
          if (res.data.success) {
            // Use filter to create a new array without the deleted post
            setPost((prevPost: any) => prevPost.filter((post: any) => post._id !== id));
  ...
  <tr className="bg-white text-center" key={post._id}>
    <td className="py-2 px-4 border-b items-center">{index + 1}</td>
    <td className="py-2 px-4 border-b font-semibold ">
        <img src={post.userId.image} className='w-10 h-10 rounded-full' alt="" />
        {post.userId?post.userId?.username:""}</td>
    <td className="py-2 px-4 border-b font-semibold">{post.description}</td>
    <td className="py-2 px-4 border-b font-semibold items-center"><img src={post.image} className="w-32 h-32 object-cover"   alt="" /></td>
    <td className="py-2 px-4 border-b font-semibold items-center">{post.createdAt.split("T")[0]}</td>
  
    <td className="py-2 px-4 border-b"><AiFillDelete onClick={()=>handleDelete(post._id)} className='cursor-pointer'/></td>
